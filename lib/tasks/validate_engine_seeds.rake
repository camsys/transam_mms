...
namespace :transam_mms do
  ...
  task :validate_engine_seeds, [:app_root] => [:environment] do |t, args|
    ...
    puts("  No db/seeds.rb file found for this engine.") || next unless File.exist?(File.join(TransamMms::Engine.root, "db", "seeds.rb"))
    puts("  No db/validate_seeds_script.rb file found for this engine.") || next unless File.exist?(File.join(TransamMms::Engine.root, "db", "validate_seeds_script.rb"))

    # Read the full db/seeds.rb file
    seeds_file = File.readlines(File.join(TransamMms::Engine.root, "db", "seeds.rb"))
    ...
    seeds_script = File.readlines(File.join(TransamMms::Engine.root, "db", "validate_seeds_script.rb"))
